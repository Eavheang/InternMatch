...
export default function Particles({
  ...
  const circleParams = (): any => {
    ...
    const translateX = 0;
    const translateY = 0;
    const particleSize = Math.random() * size + 0.5;
    const alpha = 0;
    const targetAlpha = parseFloat((Math.random() * 0.9 + 0.3).toFixed(1));
  ...
  const drawCircle = (circle: any, update = false) => {
    if (context.current) {
      ...
      gradient.addColorStop(0.5, `rgba(${hexToRgb(color)}, ${alpha * 0.3})`);
