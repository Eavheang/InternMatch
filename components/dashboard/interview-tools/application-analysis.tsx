      ...
      <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-zinc-500">
                  <div className="flex items-center gap-1.5 bg-zinc-50 px-2.5 py-1 rounded-md border border-zinc-100">
                    ...
                    <span className="font-medium text-zinc-700">{application.job.jobTitle}</span>
                  </div>
                  {application.student.university && (
                    <div className="flex items-center gap-1.5">
                      ...
                      {application.student.major && (
                        <span className="text-zinc-400">• {application.student.major}</span>
                      )}
                    </div>
                  )}
                </div>
        ...
        <div className="text-center py-16 px-4">
          ...
          <p className="text-zinc-500 max-w-md mx-auto mb-6">
            Use AI to evaluate {application.student.firstName}'s profile against the job requirements, find skill gaps, and generate interview questions.
          </p>
          <Button
            onClick={onAnalyze}
            disabled={isAnalyzing}
      ...
      <Card className="border-zinc-200 shadow-sm bg-white overflow-hidden">
        ...
        <CardContent className="pt-12 pb-6 px-8">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 pt-2">
              <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 border-emerald-100 px-3 py-1.5 text-sm font-normal">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="border-zinc-200 shadow-sm bg-white lg:col-span-1 flex flex-col">
            <CardHeader className="pb-2 pt-6 px-6">
              <CardTitle className="text-sm font-medium text-zinc-500 uppercase tracking-wider text-center">Match Score</CardTitle>
            </CardHeader>
              ...
              <div className="relative flex items-center justify-center">
                <svg className="transform -rotate-90 w-32 h-32">
                  ...
                  <circle
                    ...
                    strokeDashoffset={351.86 - (351.86 * review.matchScore) / 100}
                    className={`transition-all duration-1000 ease-out ${
                      review.matchScore >= 70 ? "text-emerald-500" : 
                      review.matchScore >= 50 ? "text-amber-500" : "text-rose-500"
                    }`}
                  />
                </svg>
          ...
          <Card className="border-zinc-200 shadow-sm bg-white lg:col-span-2 flex flex-col">
            <CardHeader className="pb-4 pt-6 px-6 border-b border-zinc-50">
              <div className="flex items-center justify-center gap-2">
                ...
                <CardTitle className="text-sm font-medium text-zinc-900">AI Executive Summary</CardTitle>
              </div>
            </CardHeader>
              ...
              {review.matchedSkills.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {review.matchedSkills.map((skill, idx) => (
                    <Badge key={idx} variant="secondary" className="bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100 px-2.5 py-1">
                      {skill}
                    </Badge>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-zinc-400 italic">No direct skill matches found.</p>
              )}
            </CardContent>
          </Card>
              ...
              {review.missingSkills.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {review.missingSkills.map((skill, idx) => (
                    <Badge key={idx} variant="outline" className="text-amber-700 border-amber-200 bg-amber-50/30 px-2.5 py-1">
                      {skill}
                    </Badge>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-zinc-400 italic">No key skills missing.</p>
              )}
            </CardContent>
          </Card>
        </div>
