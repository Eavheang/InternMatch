...
export default function CandidatesPage() {
  ...
  const [jobs, setJobs] = useState<Array<{ id: string; jobTitle: string }>>(
    []
  );
  ...
  const filteredApplications = useMemo(() => {
    return applications.filter((app) => {
      const matchesSearch =
        ...
        app.job.jobTitle.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="p-8 space-y-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            View and manage candidates who applied for your job postings.
          </p>
        </div>
      </header>
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        <div className="flex-1 text-black">
          <input
            type="text"
            placeholder="Search by name, university, or job title..."
            value={search}
        ...
        <div className="relative">
          <select
            ...
            className="w-full sm:w-64 px-4 py-2 text-black border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 appearance-none bg-white pr-10"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {application.student.university && (
                <div className="flex items-center gap-2 text-sm text-zinc-600">
                  ...
                  <span>
                    {application.student.university}
                    {application.student.major && ` • ${application.student.major}`}
                ...
                {application.application.status === "applied" && (
                  <>
                    <Button
                      ...
                      onClick={() =>
                        onStatusUpdate(application.application.id, "shortlisted")
                      }
                      disabled={isUpdating}
                    >
                      {isUpdating ? (
                ...
                {application.application.status === "shortlisted" && (
                  <>
                    <Button
                      ...
                      className="text-xs w-full"
                      onClick={() =>
                        onStatusUpdate(application.application.id, "interviewed")
                      }
                      disabled={isUpdating}
                    >
                      {isUpdating ? (
